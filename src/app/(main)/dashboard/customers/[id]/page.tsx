...
export default function CustomerDetailsPage() {
    ...
    const loadDrawingDocuments = (headers: HeadersInit) => {
        const url = `http://127.0.0.1:5000/drawings?customer_id=${id}`;
    ...
    // 🌟 CORRECTED: Handles file selection and updates state
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Iterate over the FileList and upload each file
        for (const file of Array.from(files)) {
            ...
            try {
                ...
                formData.append('customer_id', id); // Include customer ID
                
                // Upload to your backend
                const response = await fetch('http://127.0.0.1:5000/drawings/upload', {
                    method: 'POST',
                ...
                if (response.ok) {
                    ...
                    // Add the newly uploaded document to state
                    ...
                    setDrawingDocuments(prev => [newDoc, ...prev]);
                } else {
    ...
    const handleViewDrawing = (doc: DrawingDocument) => {
        // For actual files, you might open a new tab or a modal for viewing.
        alert(`Attempting to view file: ${doc.filename}. URL: http://127.0.0.1:5000${doc.url}`);
        // window.open(`http://127.0.0.1:5000${doc.url}`, '_blank');
    };
    ...
    const renderDrawingDocument = (doc: DrawingDocument) => {
        ...
        const docType = fileExtension === 'pdf' ? 'pdf' : (['png', 'jpg', 'jpeg', 'gif'].includes(fileExtension) ? 'image' : 'other');
                ...
                <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-4 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-sm text-gray-500 mt-1">Uploaded: {formatDate(doc.created_at)}</p>
                            {doc.project_id && (
                                <p className="text-xs text-blue-500 mt-1">Project ID: {doc.project_id}</p>
                            )}
                        </div>
                    </div>
                    <Button
    ...
    const loadProjectData = async (projectId: string) => {
        ...
        const projectSpecificForms = customer?.form_submissions.filter(
            form => form.project_id === projectId
        ) || [];
        setProjectForms(projectSpecificForms);

        // Filter financial docs by project
        const projectSpecificDocs = financialDocs.filter(
    ...
    return (
        <div className="min-h-screen bg-white">
            {/* HIDDEN FILE INPUT ELEMENT WITH ACCEPT ATTRIBUTE */}
            <input 
                type="file" 
                ref={fileInputRef}
                onChange={handleFileChange}
                // Filters for PDF (.pdf) and various image types (image/png, image/jpeg, etc.)
                                    ...
                                    {allProjectTypes.length > 0 ? (
                                        <div className="flex flex-wrap gap-1">
                                            ...
                                            {Array.from(new Set(allProjectTypes)).map((type, index) => (
                                                <span
                                                    key={index}
                                                    className={`inline-flex px-2 py-1 text-sm font-semibold rounded-full ${getProjectTypeColor(type)}`}
                                                >
                                                    {type}
                                                </span>
                                            ))}
                    ...
                    {customer.projects && customer.projects.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {customer.projects
                                .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
                                .map((project) => (
                                <div key={project.id} className="p-6 border rounded-lg bg-gradient-to-br from-blue-50/50 to-indigo-50/30 shadow-sm hover:shadow-md transition-all duration-200">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0 pr-4">
                                            ...
                                            <div className="space-y-2 mt-3">
                                                <div className="flex items-center space-x-2">
                                                    <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getProjectTypeColor(project.project_type)}`}>
                                                        {project.project_type}
                                                    </span>
                                                    <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getStageColor(project.stage)}`}>
                                                        {project.stage}
                                                    </span>
                                                </div>
                                            ...
                                            {project.notes && (
                                                <p className="text-sm text-gray-500 mt-3 line-clamp-2">{project.notes}</p>
                                            )}
                                        </div>
                                        <Button
                ...
                <div className="border-t border-gray-200 pt-8 mb-8">
                    ...
                    {drawingDocuments.length > 0 ? (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {drawingDocuments.map((doc) => renderDrawingDocument(doc))}
                        </div>
                    ) : (
                        <div className="bg-gray-50 p-8 rounded-lg text-center border-2 border-dashed border-gray-200">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 mb-2">No Drawings or Layouts</h3>
                            <p className="text-sm text-gray-600">
                                Upload CADs, sketches, photos, or client documentation here.
                            </p>
                        </div>
                    )}
                </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1">
                                            <div className="flex items-center space-x-3 mb-2">
                                                ...
                                                <span className={`inline-flex px-3 py-1 text-sm font-medium rounded-full ${getStageColor(job.stage)}`}>
                                                    {job.stage}
                                                </span>
                                            </div>
                ...
                <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                    ...
                    {selectedProject && (
                        <div className="space-y-6 mt-6">
                            ...
                            <section className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            <span className={`inline-flex px-3 py-1 text-sm font-medium rounded-full ${getProjectTypeColor(selectedProject.project_type)}`}>
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            <span className={`inline-flex px-3 py-1 text-sm font-medium rounded-full ${getStageColor(selectedProject.stage)}`}>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-900 mt-1">{formatDate(selectedProject.date_of_measure || '')}</p>
                                    </div>
                                    <div>
                                        <span className="text-sm text-gray-600">Created</span>
                                        <p className="text-gray-900 mt-1">{formatDate(selectedProject.created_at)}</p>
                                    </div>
                                </div>
                                {selectedProject.notes && (
                                    <div className="mt-4">
                                        <span className="text-sm text-gray-600">Notes</span>
                                        <p className="text-gray-900 mt-1 bg-white p-3 rounded">{selectedProject.notes}</p>
                                    </div>
                                )}
                            </section>
                                ...
                                {projectForms.length > 0 ? (
                                    <div className="space-y-3">
                                        {projectForms.map((form) => (
                                            <div key={form.id} className="flex items-center justify-between p-4 border rounded-lg bg-white hover:bg-gray-50 transition">
                                                <div>
                            ...
                            <section>
                                <h3 className="text-lg font-semibold text-gray-900 mb-4">
                                    Financial Documents ({projectDocs.length})
                                </h3>
                                {projectDocs.length > 0 ? (
                                    <div className="space-y-3">
                                        {projectDocs.map((doc) => (
                                            <div key={doc.id} className="flex items-center justify-between p-4 border rounded-lg bg-white hover:bg-gray-50 transition">
                                                <div className="flex items-center space-x-3">
                                                    <div className="p-2 bg-blue-50 rounded">
                                                        {FINANCIAL_DOCUMENT_ICONS[doc.type]}
                                                    </div>
                                                    <div>
                                                        <h4 className="font-medium text-gray-900">{doc.title}</h4>
                                                        <p className="text-sm text-gray-500">Created: {formatDate(doc.created_at)}</p>
                                                        {doc.total && (
                                                            <p className="text-sm text-blue-600 font-medium">£{doc.total.toFixed(2)}</p>
                                                        )}
                                                    </div>
                                                </div>
                                                <Button
